...
const LoginForm = ({ setWantsToLogin }) => {
  ...
  return (
    <div className="flex flex-col ml-20 mt-40 font-bold">
      <h1 className="text-7xl font-black text-white italic">Log in</h1>
      <p className="mt-2 text-white text-sm font-light"></p>
      <form
      ...
      >
        <label
          htmlFor="email"
          className="text-l text-center font-semibold text-white"
        >
          Wallet
        </label>
        <input
          ...
          className="placeholder:italic placeholder:text-slate-100 block bg-gray-900 bg-opacity-5 h-12 my-2 w-3/4 border border-slate-300 rounded-md py-2 pl-3 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm text-white text-center"
        />
        {address &&
          (!isWrongNetwork ? (
            <>
